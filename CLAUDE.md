...
Запуск — четыре процесса, каждый в своём окне, из папки `build`:
```
...
**Миграций БД нет.** Таблицы создаются через `CREATE TABLE IF NOT EXISTS` в конструкторах репозиториев. Если меняется схема — существующий `.db` файл не обновится, его надо удалить вручную из `build/`. Всегда предупреждай об этом при изменении схемы.
...
- **Только Windows (пока).** Сетевой код переведён на слой абстракции `common/include/PlatformSocket.h` (`socket_t`, `CloseSocket`, `GetLastSocketError`, `IsTimeoutError`, `SetRecvTimeout`, `SocketLibraryGuard`), но собиралось и проверялось это пока только на Windows. Линковка `ws2_32` в CMake уже условная (`if(WIN32)`), `Threads::Threads` добавлена для будущей Linux-сборки. Сама сборка и прогон на Linux ещё не сделаны.
...
1. **Портирование сервера на Linux** — слой абстракции над сокетами в `common/include/PlatformSocket.h` готов и используется во всех сервисах (ветка `dev`), Windows-сборка проверена. Осталось собрать и прогнать на Linux. Делать до конфигов и TLS, иначе придётся переделывать дважды.
