...
- **Ролевая модель — без модерации и IP-банов.** Роли, права, оверрайды по каналам (видимость, вход, отправка сообщений) и создание каналов уже проверяются по правам. Не сделано: кик/мьют по каналам, бан по IP на уровне сервера, UI для управления ролями/правами (пока только `test_client`/`FreeCord.ConsoleTest`). Отдельно: с появлением проверки `ManageChannel` на создание комнаты обычный `guest` больше не может создавать комнаты по умолчанию — раньше мог любой залогиненный.
