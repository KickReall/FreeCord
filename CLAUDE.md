...
Все четыре сервиса при старте читают `config.json` из своей рабочей директории. CMake копирует его из корня репозитория в `build-windows/`/`build-linux/` при каждой сборке (таргет `copy_config`) — именно туда, откуда по инструкции выше и запускаются `.exe`. Файла нет или он битый — сервис сразу падает с понятным сообщением об ошибке, никаких скрытых дефолтов в коде нет.
...
2. ~~**JSON-конфигурация**~~ — сделано в ветке `dev`. `config.json` в корне репозитория, `common/include/Config.h` + `LoadConfig`, все четыре сервиса читают порты/пути к БД/адрес хоста оттуда вместо констант. Ветка ещё не влита в `main`.
